...
def load_chats():
    with open("chats.csv", "rb") as saved_chats:
        reader = csv.reader(saved_chats)
...
def read_message():
    ...
    with open("chats.csv", "a") as chats:
        writer = csv.writer(chats)
        writer.writerow([spy.name, present_time, "Sent by "+friend_list[sender_friend].name])
    friend_list[sender_friend].chats.append(chat)


def chat_history():
    friend_pos = select_a_friend()
    friend_name = friend_list[friend_pos].name
    for x in range(len(chats)):
        if chats[x] == friend_name:
            print '\x1b[6;30;42m' + 'Time: ' + chats[x].time + '\x1b[0m', \
                '\x1b[6;34;0m' + "Message: " + chats[x].message + '\x1b[0m', \
                '\x1b[6;34;0m' + "Sent By: " + chats[x].sent_by + '\x1b[0m'
